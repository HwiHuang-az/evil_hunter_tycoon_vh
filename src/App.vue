<template>
  ...
  <div v-if="isOpened" class="black-bg" @click="isOpened = false">
    <div class="white-bg">
      ...
      <p>
        Tốc đánh cuối = Tốc đánh vũ khí * (1 - chỉ số - tính cách - bí pháp -
        liên minh - trang bị thú cưng - trang bị) / (1 + Fury + Quicken)
      </p>
      <p>
        {{ final_speed }} = {{ weapon_speed }} * (1 - {{ stat }} -
        {{ personality }} - {{ secret / 100 }} - {{ union_speed / 100 }} -
        {{ pet_equipment / 100 }} - {{ equip_speed / 100 }}) / (1 +
        {{ fury - 1 }} + {{ quicken - 1 }})
      </p>
    </div>
  </div>

  <a href="/evil_hunter_tycoon_vh/"
    ><img
      alt="Evil Hunter Tycoon Việt hóa"
      class="title_img"
      src="./assets/eht_title.png"
  /></a>
  <div>
    <a
      class="hits"
      href="https://hits.sh/hwihuang-az.github.io/evil_hunter_tycoon_vh/"
    >
      <img
        alt="Lượt truy cập"
        src="https://hits.sh/hwihuang-az.github.io/evil_hunter_tycoon_vh.svg?view=today-total"
      />
    </a>
  ...
  <div class="tab-buttons">
    ...
    <button
      :class="{ active: activeTab === 'rune' }"
      @click="activeTab = 'rune'"
    >
      Thông tin Rune
    </button>
    <button
      :class="{ active: activeTab === 'dogam' }"
      @click="activeTab = 'dogam'"
    >
      Bộ sưu tập / Tỉ lệ
    </button>
    ...
    <button
      :class="{ active: activeTab === 'equipGrade' }"
      @click="activeTab = 'equipGrade'"
    >
      Hạng tùy chọn trang bị
    </button>
    <button
      :class="{ active: activeTab === 'relic' }"
      @click="activeTab = 'relic'"
    >
      Di vật
    </button>
    <button
      :class="{ active: activeTab === 'unique' }"
  ...
  <div v-show="activeTab === 'attack'">
    <div class="calc-scroll">
      <table class="attack-table">
        ...
        <tbody>
          ...
          <td v-show="job === 0">
            <select
              v-model.trim.number="weapon_speed"
              @change="weapon_speed_cal"
            >
          ...
          <td>
            <select
              v-model.trim.number="personality"
              @change="weapon_speed_cal"
            >
              <option disabled value="">Chọn</option>
          ...
          <td>
            <input
              v-model.trim.number="union_speed"
              max="5"
              min="0"
              onfocus="this.value=''"
              placeholder="Nhập(%)"
          ...
          <td>
            <select
              v-model.trim.number="pet_equipment"
              @change="weapon_speed_cal"
            >
              <option disabled value="">Chọn</option>
    ...
    <div class="equip">
      ...
      <input
        v-model.trim.number="equip_speed"
        max="100"
        min="0"
        placeholder="Nhập(%)"
        type="number"
    ...
    <div class="final">
      ...
      <input
        v-model.trim.number="final_speed"
        max="3"
        min="0.25"
        placeholder="Nhập(%)"
        step="0.01"
    ...
    <ol>
      ...
      <li>
        Chọn vũ khí, chỉ số, tính cách, Quicken và Fury; sau đó nhập Bí pháp và
        tốc đánh Liên Minh.
      </li>
      <li>
        Nếu trang bị Rune Fury Lv.1/2/3, hãy chọn Fury Lv.11/12/13 tương ứng.
      </li>
      <li>
        Dựa trên <span class="final_inline">tốc đánh cuối</span> mặc định 0,25,
        công cụ sẽ tính
        <span class="equip_inline">tốc đánh cần có từ Rune và trang bị</span>.
        Bạn cũng có thể sửa trực tiếp
        <span class="final_inline">tốc đánh cuối</span>.
      </li>
      <li>
        Khi sửa <span class="equip_inline">tốc đánh từ Rune và trang bị</span>,
        công cụ sẽ tính lại <span class="final_inline">tốc đánh cuối</span>.
      </li>
      <li>Kết quả không được làm tròn để giữ độ chính xác.</li>
      <li>
        Ví dụ: nếu kết quả cần 82,955% tốc đánh từ Rune và trang bị, bạn phải
        đạt ít nhất 83%.
      </li>
      <li>
        Ví dụ: tốc đánh thực là 0,255 có thể hiển thị 0,25 trong game, nhưng sát
        thương mỗi giây vẫn được tính theo 0,255.
      </li>
    </ol>
  </div>
  ...
  <div v-show="activeTab === 'move'">
      ...
      <tbody>
        ...
        <tr>
          ...
          <td class="slider-cell">
            <input
              ...
              max="50"
              step="1"
              v-model.number="moveBuffs.building"
            />
            <input
              type="number"
              min="0"
        ...
        <tr>
          ...
          <td class="slider-cell">
            <input
              ...
              max="15"
              step="1"
              v-model.number="moveBuffs.secret"
            />
            <input
              type="number"
              min="0"
        ...
        <tr>
          ...
          <td>
            <input
              type="number"
              v-model.number="moveBuffs.equipmentTotal"
              placeholder="Nhập"
              class="full-width-input"
            />
          </td>
        ...
        <tr>
          <td>
            Đại Ác Ma (Vũ khí Nguyên Thủy, Khởi Nguyên, Hỗn Mang, Vực Thẳm)
          </td>
          <td>
    ...
    <div class="move-speed-results">
      ...
      <div class="result-item result-note">
        <span class="label"
          >Khuếch đại Công của Thần Cuồng Phong (tối đa 30%)</span
        >
      </div>
      <div class="result-item result-note">
        <span class="label"
          >Khuếch đại Công của Chân Thần Cuồng Phong (tối đa 40%)</span
        >
      </div>
    </div>
      ...
      <table>
        ...
        <tbody>
          <tr v-for="rate in requirementRates" :key="rate">
            ...
            <td :class="{ met: isMet(requiredSpeed(30, rate)) }">
              {{ deficit(requiredSpeed(30, rate)) }} ({{
                currentAmp(30, rate)
              }}%)
            </td>
            <td>{{ requiredSpeed(40, rate) }}</td>
            <td :class="{ met: isMet(requiredSpeed(40, rate)) }">
              {{ deficit(requiredSpeed(40, rate)) }} ({{
                currentAmp(40, rate)
              }}%)
            </td>
          </tr>
  ...
  <div v-show="activeTab === 'kills'">
    <div class="kills-calculator-container">
        ...
        <tbody>
          <tr v-for="(record, index) in killsRecords" :key="index">
            ...
            <td>
              <input
                type="text"
                :value="record.kph !== null ? record.kph : ''"
                readonly
                class="kpm-display-input"
              />
            </td>
          </tr>
        </tbody>
      ...
      <div class="kills-buttons">
        ...
        <button @click="resetExcludingLastKillsRecord">
          Giữ bản ghi cuối và đặt lại
        </button>
      </div>
      <ol class="kills-guide">
        <h3>⭐ Hướng dẫn sử dụng</h3>
        <li>
          Ở dòng đầu tiên, nhập tháng, ngày, giờ, phút hiện tại và tổng số quái
          của Thị trưởng.
        </li>
        <li>
          Nút <b>Điền giờ hiện tại</b> sẽ điền thời gian vào dòng trống kế tiếp;
          nếu hết dòng trống, hệ thống tự thêm dòng mới.
        </li>
        <li>
          Sau một khoảng thời gian, nhập thời gian và tổng số quái mới ở dòng kế
          tiếp. Công cụ sẽ tính số quái hạ được mỗi giờ dựa trên chênh lệch giữa
          hai bản ghi.
        </li>
        <li>Dùng nút <b>Thêm dòng</b> khi muốn ghi thêm dữ liệu.</li>
        <li>
          Nút <b>Xóa toàn bộ bảng</b> sẽ xóa mọi bản ghi và đưa bảng về bốn dòng
          trống ban đầu.
        </li>
        <li>
          Nút <b>Giữ bản ghi cuối và đặt lại</b> sẽ giữ bản ghi hợp lệ gần nhất
          ở dòng đầu rồi xóa các dòng còn lại. Nếu bản ghi cuối chưa đủ dữ liệu,
          toàn bộ bảng sẽ được đặt lại.
        </li>
      </ol>
    </div>
  </div>
    ...
    <div class="rune-container">
      <div class="rune-floor-finder">
        ...
        <input
          ...
          min="1"
          max="300"
          v-model.number="currentFloor"
          placeholder="Ví dụ: 235"
        />
        <div class="rune-floor-result" v-if="currentFloorStages">
          <span
            v-for="s in currentFloorStages"
            :key="s.name"
            class="floor-chip"
            :style="s.style"
          >
        ...
        <div class="rune-floor-result rune-floor-empty" v-else>
          Nhập một tầng từ 1 đến 300 để đánh dấu vị trí tương ứng trong bảng.
        </div>
      </div>

      <h3>Tầng nhận Rune</h3>
      <p class="rune-note">
        Mỗi ô tương ứng một khoảng cấp. Màu càng đậm thì cấp càng cao.
      </p>
          ...
          <table class="rune-matrix">
            <thead>
              <tr>
                ...
                <th v-for="h in blockHeaders(block)" :key="h.from">
                  {{ h.from }}<br />~{{ h.to }}
                </th>
              </tr>
            </thead>
            <tbody>
              <tr v-for="row in runeRows" :key="row.name">
                ...
                <td
                  v-for="cell in blockCells(row, block)"
                  :key="cell.from"
                  :colspan="cell.span"
                  :class="{
                    'rune-empty': cell.stage === null,
                    'rune-active': isActiveCell(cell),
                  }"
                  :style="
                    cell.stage === null ? null : bandStyle(row, cell.stage)
                  "
                >
                  {{
      ...
      <p class="rune-note">
        Đây là <b>giá trị tối đa</b> theo từng cấp. Các tùy chọn trong cùng một
        nhóm có giá trị như nhau.
      </p>
      <div class="rune-scroll">
      ...
      <div class="rune-group-list">
        <div
          v-for="g in runeOptionGroups"
          :key="g.label"
          class="rune-group-item"
        >
          <div class="rune-group-head">
            <span class="rune-group-badge">{{ g.label }}</span
            >{{ g.title }} ({{ g.options.length }} loại)
          </div>
      ...
      <ol class="rune-guide">
        <h3>⭐ Ghi Chú</h3>
        <li>
          Tất cả Rune chỉ xuất hiện tại <b>Sân trước Thành Ma Vương</b>. Tầng
          càng cao thì cấp Rune xuất hiện càng cao.
        </li>
        <li>
          Các Rune kỹ năng cùng bậc xuất hiện ở cùng tầng, không phụ thuộc loại
          kỹ năng.
        </li>
        <li>
          Rune chỉ số tăng 1 cấp sau mỗi 10 tầng từ tầng 1–100 và đạt cấp 10 ở
          tầng 91–100. Tầng 101–200 giữ cấp 10; từ tầng 201, cứ 20 tầng tăng 1
          cấp đến cấp 15.
        </li>
        <li>
          Riêng Rune kỹ năng bậc 3 tiếp tục ở cấp 3 tại tầng 201–210, sau đó
          tăng theo mỗi 20 tầng.
        </li>
        <li>
          Cấp 11–15 chưa có dữ liệu chính thức về chỉ số tối thiểu nên bảng chỉ
          hiển thị giá trị tối đa.
        </li>
        <li>
          Nhóm A tăng 1 điểm tối đa sau mỗi 2 cấp đến cấp 10; từ cấp 11, mỗi cấp
          tăng 1 điểm.
        </li>
        <li>
          Nguồn: cộng đồng chính thức Evil Hunter Tycoon (Bảng tùy chọn Rune /
          Tầng xuất hiện Rune kỹ năng / Thông tin Rune tầng 201–300)
        </li>
      </ol>
    </div>
  </div>
  ...
  <div v-show="activeTab === 'dogam'">
    <div class="dogam-container">
      ...
      <p class="rune-note">
        Đây là tổng tỉ lệ của các vật phẩm bạn chưa sở hữu. Giá trị càng cao thì
        cơ hội nhận được vật phẩm mới trong một lần mở càng lớn.
      </p>
      <table class="dogam-priority">
        ...
        <tbody>
          ...
          >
            ...
            <td>
              {{ b.expected === null ? "Hoàn tất" : b.expected + " lần" }}
            </td>
          </tr>
        </tbody>
      </table>
      ...
      <div class="dogam-sections">
        <button
          v-for="s in dogamSectionList"
          :key="s.id"
          :class="{ active: dogamSection === s.id }"
          @click="dogamSection = s.id"
        >
          {{ s.label }}
        </button>
      </div>
      ...
      <div class="dogam-tools">
        <label class="dogam-filter">
          <input type="checkbox" v-model="dogamHideOwned" /> Chỉ hiện vật phẩm
          chưa có
        </label>
          ...
          <template v-for="g in visibleSetGroups" :key="g.no">
            <tr class="set-head">
              <td colspan="5">
                ...
                <span
                  class="set-progress"
                  :class="{ done: g.ownedCount === g.total }"
                  >{{ g.ownedCount }}/{{ g.total }}</span
                >
              </td>
            </tr>
            <tr
              ...
              :key="i"
              :class="{ owned: isOwned(it.key) }"
            >
              <td>{{ it.cat }}</td>
              <td class="item-name">{{ it.name }}</td>
              <td :class="{ 'src-box': isBoxSource(it.src) }">
                {{ it.src || "-" }}
              </td>
              ...
              <td>
                <input
                  type="checkbox"
                  :checked="isOwned(it.key)"
                  @change="toggleOwned(it.key)"
                />
              </td>
            </tr>
          </template>
          <tr v-if="!visibleSetGroups.length">
            <td colspan="5" class="dogam-empty">
              Không có vật phẩm để hiển thị.
            </td>
          </tr>
        </tbody>
      </table>
      ...
      <table
        v-else-if="dogamSection === 'Rương Bí Mật'"
        class="dogam-table dogam-chonbi-table"
      >
        <thead>
          <tr>
            <th>Phân loại</th>
        ...
        <tbody>
          <tr
            v-for="(it, i) in visibleChonbiItems"
            :key="i"
            :class="{ owned: it.key && isOwned(it.key) }"
          >
            <td>{{ it.cat }}</td>
            <td class="item-name">{{ it.name }}</td>
            <td>{{ it.rate }}%</td>
          ...
          <tr v-if="!visibleChonbiItems.length">
            <td colspan="4" class="dogam-empty">
              Không có vật phẩm để hiển thị.
            </td>
          </tr>
        </tbody>
      ...
      <ol class="dogam-guide">
        <h3>⭐ Hướng dẫn sử dụng</h3>
        <li>
          Lựa chọn của bạn được lưu trong trình duyệt và vẫn còn khi quay lại
          trang.
        </li>
        <li>
          Vật phẩm đánh dấu trong Hộp ngoại trang lấp lánh A/B hoặc Rương bí mật
          Thị trưởng sẽ tự đồng bộ với Bộ sưu tập và ngược lại.
        </li>
        <li>
          Nên mở rương có <b>tỉ lệ vật phẩm còn thiếu</b> cao nhất để tăng cơ
          hội nhận vật phẩm mới.
        </li>
        <li>
          Rương bí mật Thị trưởng chỉ ghi nhận ngoại trang vào Bộ sưu tập; tiền
          tệ và vật phẩm không thể sưu tầm không được tính.
        </li>
        <li>
          Vật phẩm nhận từ Sân trước Thành ma vương, đá quý hoặc răng hàm cần
          được đánh dấu thủ công.
        </li>
        <li>
          Khi dùng lần đầu, cách nhanh nhất là chọn tất cả rồi bỏ chọn những vật
          phẩm bạn chưa có.
        </li>
      </ol>
    </div>
  ...
  <div v-show="activeTab === 'pet'">
    <div class="pet-container">
      ...
      <p class="rune-note">
        Mô tả kỹ năng áp dụng và nghề áp dụng được hiển thị chung trong một ô.
      </p>

      <div v-for="set in petEquipSets" :key="set.name" class="pet-set">
        <div class="pet-set-head" :style="petColor(set, 78)">
          {{ set.name }}
        </div>
        <div v-for="item in set.items" :key="item.part" class="pet-item">
          <div class="pet-item-head">
            <span class="pet-part" :style="petColor(set, 88)">{{
              item.part
            }}</span>
      ...
      <ol class="pet-guide">
        ...
        <li>
          Chỉ những tùy chọn có đối tượng áp dụng cụ thể mới hiển thị mô tả và
          nghề bên dưới.
        </li>
  ...
  <div v-show="activeTab === 'formation'">
    <div class="formation-container">
      <div class="formation-form">
        ...
        />
        <input
          v-model.trim="newUnit.job"
          type="text"
          list="formation-job-list"
          placeholder="Nghề"
          @keyup.enter="addUnit"
        />
        ...
        <label class="formation-reset-job">
          <input type="checkbox" v-model="resetJobOnAdd" /> Đặt lại nghề sau khi
          thêm
        </label>
      </div>
      ...
      <div v-for="side in boardOrder" :key="side" class="formation-side">
        <div class="formation-side-head">
          <span class="formation-side-name" :class="side">{{
            side === "mine" ? "Phe Ta Đội Hình" : "Đối Thủ Đội Hình"
          }}</span>
          <span class="formation-count"
            >{{ unitCount(side) }} / {{ maxUnits }}</span
          >
          <button class="formation-share" @click="exportSide(side)">
            Xuất mã
          </button>
          <button class="formation-share" @click="openLoad(side)">
            Tải mã
          </button>
          <button class="formation-clear" @click="clearSide(side)">
            Xóa đội hình
          </button>
        </div>
        <div v-if="shareBox.side === side" class="formation-share-box">
          <textarea
            v-model="shareBox.text"
            class="formation-share-text"
            rows="2"
            :placeholder="
              shareBox.mode === 'export' ? '' : 'Dán mã đội hình đã nhận'
            "
            @focus="$event.target.select()"
          ></textarea>
          <div class="formation-share-actions">
            <button v-if="shareBox.mode === 'export'" @click="copyShare">
              Sao chép
            </button>
            <button v-else @click="loadSide(side)">Tải</button>
            <button @click="shareBox.side = null">Đóng</button>
          </div>
          <p v-if="shareBox.message" class="formation-share-msg">
            {{ shareBox.message }}
          </p>
        </div>
        <div class="formation-presets">
          ...
          <div
            v-for="(preset, i) in formationPresets[side]"
            :key="i"
            class="formation-preset-row"
            :class="{ filled: !!preset.code }"
          >
            <input
              v-model="preset.name"
              class="formation-preset-name"
              maxlength="16"
              :placeholder="'Mẫu Lưu ' + (i + 1)"
              @change="savePresets"
            />
            <button class="formation-preset-save" @click="savePreset(side, i)">
              Lưu
            </button>
            <button
              class="formation-preset-load"
              :disabled="!preset.code"
              @click="applyPreset(side, i)"
            >
              Tải
            </button>
          </div>
          <p v-if="presetMessage.side === side" class="formation-share-msg">
            {{ presetMessage.text }}
          </p>
        </div>
        <div
          class="formation-board"
          :ref="(el) => setBoardRef(side, el)"
          :style="boardStyle"
        >
          <div
            v-for="tile in tilesOf(side)"
            :key="tile.i"
            class="ftile"
            :class="{ occupied: !!tile.unit }"
          ...
          >
            ...
            <div
              v-else
              class="funit"
              :class="{ dragging: isDragging(side, tile.i) }"
              @pointerdown="startDrag(side, tile, $event)"
            >
              <span class="funit-name">{{ tile.unit.name }}</span>
              <span
                class="funit-job"
                :style="{ color: jobColor(tile.unit.job) }"
                >{{ tile.unit.job }}</span
              >
              <button
                class="funit-del"
                @pointerdown.stop
                @click.stop="removeUnit(side, tile.i)"
              >
                ×
              </button>
            </div>
      ...
      <div v-if="drag" class="funit drag-ghost" :style="ghostStyle">
        ...
        <span class="funit-job" :style="{ color: jobColor(drag.unit.job) }">{{
          drag.unit.job
        }}</span>
      </div>
      ...
      <ol class="formation-guide">
        <h3>⭐ Hướng dẫn sử dụng</h3>
        <li>
          Nhập tên và nghề, chọn <b>Phe Ta / Đối Thủ</b>, rồi thêm nhân vật; hệ
          thống sẽ điền vào ô trống đầu tiên.
        </li>
        <li>
          Nếu bỏ trống tên, hệ thống tự đặt lần lượt là
          <b>Đồng Minh 1, Đồng Minh 2</b> hoặc <b>Kẻ Địch 1, Kẻ Địch 2</b>.
        </li>
        <li>
          Mỗi phe bố trí được <b>tối đa {{ maxUnits }} người</b>. Bàn có 16 ô
          nhưng chỉ xếp được 10 người.
        </li>
        <li>
          Nếu tắt <b>Đặt lại nghề sau khi thêm</b>, nghề đã chọn sẽ được giữ lại
          khi thêm nhiều nhân vật liên tiếp.
        </li>
        <li>
          Có thể <b>kéo</b> nhân vật sang ô khác bằng chuột hoặc thao tác cảm
          ứng.
        </li>
        <li>
          Nếu thả vào ô đã có nhân vật, hai nhân vật sẽ <b>đổi vị trí</b>.
        </li>
        <li>Có thể kéo nhân vật qua lại giữa đội hình Phe Ta và Đối Thủ.</li>
        <li>
          Số ô giống trong game; <b>ô số 1 của hai bên đối diện nhau</b> ở hàng
          đầu.
        </li>
        <li>
          Đội hình được lưu trong trình duyệt và vẫn còn khi bạn quay lại.
        </li>
        <li>
          Dùng <b>Xuất mã</b> để chia sẻ đội hình; dán mã đã nhận rồi chọn
          <b>Tải</b> để khôi phục. Đội hình Phe Ta và Đối Thủ được xử lý riêng.
        </li>
        <li>
          Bạn có 5 <b>mẫu lưu</b> cho đội hình thường dùng. Nút <b>Lưu</b> ghi
          đội hình hiện tại vào ô đã chọn; tên mẫu do bạn nhập. Các mẫu cũng
    ...
    <div class="league-container">
      ...
      <div class="league-finder">
        ...
        <input
          ...
          placeholder="Ví dụ: 2450"
        />
        <div v-if="myTier" class="league-result">
          <span
            class="league-result-tier"
            :style="{ backgroundColor: leagueColor(myTier.group) }"
            >{{ myTier.tier }}</span
          >
          <span class="league-result-honor"
            >Điểm danh dự {{ myTier.honor }}</span
          >
        </div>
        <div v-else class="league-result league-result-empty">
          {{
            myRating
              ? "Dưới Đồng IV nên chưa đủ điều kiện nhận thưởng."
              : "Nhập điểm xếp hạng để xác định bậc hiện tại."
          }}
        </div>
      </div>
      ...
      <table class="league-table reward-table">
        ...
        <thead>
          <tr>
            <th colspan="4" class="reward-title ranking">
      ...
      <table class="league-table reward-table">
        ...
        <thead>
          <tr>
            <th colspan="4" class="reward-title joining">
  ...
  <div v-show="activeTab === 'equipGrade'">
    <div class="equip-grade-container">
      <div class="equip-grade-tabs">
        <button
          v-for="g in equipGrades"
          :key="g.id"
          :class="{ active: equipGradeTier === g.id }"
          @click="equipGradeTier = g.id"
        >
          {{ g.id }}
        </button>
      </div>
        ...
        <table class="equip-grade-table">
          <thead>
            <tr>
              ...
              <th
                v-for="h in equipTierHeaders"
                :key="h"
                :style="equipGradeHeadStyle(h)"
              >
                {{ h }}
              </th>
            </tr>
          ...
          <tbody>
            <tr v-for="(row, i) in currentEquipGrade.common" :key="i">
              ...
              <td
                v-for="(val, j) in row.v"
                :key="j"
                :style="equipGradeCellStyle(j)"
              >
                {{ val }}
              </td>
            </tr>
      ...
      <div class="rune-scroll">
        <table class="equip-grade-table">
          <thead>
            <tr>
              <th class="equip-opt-head weapon">
                Tùy chọn dành riêng cho vũ khí
              </th>
              <th
                v-for="h in equipTierHeaders"
                :key="h"
      ...
      <ol class="rune-guide">
        <h3>⭐ Ghi Chú</h3>
        <li>
          Các giá trị là <b>mức tùy chọn tối đa</b> có thể xuất hiện ở từng phẩm
          chất (MAX–C).
        </li>
        <li>
          Các tùy chọn trong cùng một hàng có giá trị giống nhau ở từng phẩm
          chất.
        </li>
      </ol>
    </div>
  ...
  <div v-show="activeTab === 'relic'">
    <div class="relic-container">
      ...
      <p class="rune-note">
        Tỉ lệ thu thập của mọi Di vật là 1/30. Chỉ số buff được liệt kê theo cấp
        Di vật 1–10.
      </p>
      <div class="rune-scroll">
      ...
      <p class="rune-note">
        Mỗi Di vật được kích hoạt khi nhận món đầu tiên; bảng thể hiện số lượng
        cần cho các lần nâng cấp sau.
      </p>
      <div class="rune-scroll">
      ...
      <ol class="rune-guide">
        <h3>⭐ Ghi Chú</h3>
        <li>
          Di vật dư sau khi đạt cấp tối đa (Lv.10) có thể đổi trong mục
          <b>Đổi Di vật dư</b>, mỗi món nhận 5 token Di vật.
        </li>
  ...
  <div v-show="activeTab === 'unique'">
    <div class="unique-container">
      <div class="unique-tabs">
        <button
          v-for="m in uniqueModes"
          :key="m"
          :class="{ active: uniqueMode === m }"
          @click="uniqueMode = m"
        >
          {{ m }}
        </button>
      ...
      <p class="rune-note">
        Rơi trong chế độ {{ uniqueMode }} từ cấp được ghi và tiếp tục rơi ở các
        cấp cao hơn. Tỉ lệ rơi tăng theo thứ tự: quái thường &lt; Boss &lt; Boss
        Thế Giới.
      </p>

      <div
        v-for="stage in stagesOf(uniqueMode)"
        :key="stage"
        class="unique-stage-group"
      >
          ...
          <table class="unique-table">
            ...
            <tbody>
              <tr v-for="u in uniquesOf(uniqueMode, stage)" :key="u.name">
                ...
                <td>{{ u.part }}</td>
                <td class="unique-option">
                  {{ u.option || "Chưa có thông tin tùy chọn" }}
                </td>
              </tr>
            </tbody>
          </table>
      ...
      <ol class="rune-guide">
        <li>
          Trang bị Độc nhất có <b>tổng cộng 5 tùy chọn</b>. Ngoài tùy chọn
          riêng, các dòng còn lại là tùy chọn ngẫu nhiên.
        </li>
        <li>
          Có thể thay đổi tùy chọn riêng bằng <b>Phước Lành Thiên Thần</b> hoặc
          <b>Phước Lành Đại Thiên Thần</b>. Phước Lành Thiên Thần có tỉ lệ thành
          công 25% và mua bằng 1.000 điểm Chiến Thắng; Phước Lành Đại Thiên Thần
          là vật phẩm trả phí.
        </li>
        <li>
          Một số trang bị Độc nhất (Giáp Nhẹ Cuồng Phong, Mũ Juggernaut và Chân
          Giáp Nhẹ Cuồng Phong) có thêm một <b>tùy chọn cố định ở dòng đầu</b>,
          ngoài tùy chọn riêng.
        </li>
        <li>
          Trang bị Độc nhất không nhận các tùy chọn ngẫu nhiên dành riêng cho vũ
          khí.
        </li>
        <li>
          Trang bị Độc nhất ở Boost Vực Thẳm cao hơn Boost Hỗn Mang một bậc về
          cấp vật phẩm, chỉ số và phạm vi tùy chọn.
        </li>
      </ol>
      ...
      <p class="rune-note">
        Ngày của v1.331 và v1.367 lấy theo thông báo trước; bản cập nhật thực tế
        được phát hành trong vài ngày sau đó.
      </p>
    </div>
  </div>

  <div class="maker">
    <div v-for="(credit, i) in currentCredits" :key="i">
      {{ credit.role }} bởi {{ credit.name }}
    </div>
  </div>
</template>
...
export default {
  ...
  data() {
    return {
      ...
      tabCredits: {
        attack: [
          { role: "Việt hóa", name: "HwiHuang" },
          { role: "Nguồn", name: "Chú Phan Văn Viên (NG Viên Ú)" },
        ],
        move: [
          { role: "Việt hóa", name: "HwiHuang" },
          { role: "Nguồn", name: "Chú Phan Văn Viên (NG Viên Ú)" },
      ...
      runeOptionGroups: [
        ...
        },
        {
          ...
          options: [
            "Tốc chạy %",
            "Sát thương Hút máu %",
            "Nhận Sát thương 25% Giảm Tỉ lệ %",
            "Sát thương cộng thêm khi tấn công %",
            "Khi Tấn Công 3% Tâm trạng Hồi phục Tỉ lệ %",
            "Khi Tấn Công 1% Thể lực Hồi phục Tỉ lệ %",
            "Khi Tấn Công 3% Đói Hồi phục Tỉ lệ %",
          ],
        ...
        },
        {
          ...
          options: [
            "Tâm trạng Lượng tiêu hao %",
            "Thể lực Lượng tiêu hao %",
            "Đói Lượng tiêu hao %",
            "Nhận Kinh nghiệm %",
          ],
        ...
        },
        {
          ...
          options: [
            ...
            "Tất cả Thủ %",
            "HP %",
            "Tỉ lệ nhận gấp đôi Vàng %",
            "Nguyên Liệu Thêm Nhận Tỉ lệ %",
          ],
      ...
      skillRuneList: [
        {
          tier: "Rune kỹ năng bậc 1",
          runes: ["Fury", "Holy Light", "Multi Shot", "Thunderbolt"],
        },
        {
          tier: "Rune kỹ năng bậc 2",
          runes: [
            "Dual Weapon",
            "Death Coil",
            "Pulverize",
      ...
      // Formation Editor
      formation: {
        mine: new Array(16).fill(null),
        enemy: new Array(16).fill(null),
      },
  ...
  computed: {
    totalMoveSpeed() {
      ...
      return Object.values(this.moveBuffs).reduce(
    ...
    boxStats() {
      return boxes.map((box) => {
        ...
        const remainRate = remain.reduce(
    ...
    visibleSetGroups() {
      ...
      dogamSets.forEach((item) => {
        ...
        if (!this.dogamHideOwned || !this.dogamOwned[item.key])
    ...
    visibleBoxItems() {
      ...
      return this.dogamHideOwned
        ? items.filter((i) => !this.dogamOwned[i.key])
        : items;
    },
    visibleChonbiItems() {
    ...
    currentEquipGrade() {
      return (
        equipGrades.find((g) => g.id === this.equipGradeTier) || equipGrades[0]
      );
    },
    stagesOf() {
      return (mode) => {
        const seen = [];
        uniques.forEach((u) => {
          if (u.mode === mode && seen.indexOf(u.stage) === -1)
            seen.push(u.stage);
        });
        return seen;
      };
    ...
    uniquesOf() {
      return (mode, stage) =>
        uniques.filter((u) => u.mode === mode && u.stage === stage);
    },
    equipGradeHeadStyle() {
      return (h) => {
  ...
  methods: {
    ...
    autoName(side) {
      ...
      const used = new Set(
        this.formation[side].filter(Boolean).map((u) => u.name),
      );
      let n = 1;
    ...
    copyShare() {
      ...
      const done = () => {
        this.shareBox.message = "Đã sao chép.";
      };
      if (navigator.clipboard && navigator.clipboard.writeText) {
        navigator.clipboard
          ...
          .then(done)
          .catch(() => {
            this.shareBox.message =
              "Sao chép thất bại. Hãy chọn và sao chép thủ công.";
          });
      } else {
    ...
    savePresets() {
      localStorage.setItem(
        this.presetLocalStorageKey,
        JSON.stringify(this.formationPresets),
      );
    },
    loadPresets() {
    ...
    isDragging(side, index) {
      return (
        !!this.drag && this.drag.side === side && this.drag.index === index
      );
    },
    ...
    moveUnit(fromSide, fromIndex, toSide, toIndex) {
      ...
      if (
        fromSide !== toSide &&
        !displaced &&
        this.unitCount(toSide) >= MAX_UNITS
      ) {
        this.formationMessage =
          this.sideLabel(toSide) +
          " chỉ được bố trí tối đa " +
          MAX_UNITS +
          " người.";
        return;
    ...
    setSectionOwned(value) {
      ...
      if (this.dogamSection === "sets") {
        ...
        keys = (this.dogamSection === "Lấp LánhA" ? boxAItems : boxBItems).map(
          (i) => i.key,
        );
      }
      keys.forEach((key) => {
    ...
    saveDogamOwned() {
      localStorage.setItem(
        this.dogamLocalStorageKey,
        JSON.stringify(this.dogamOwned),
      );
    },
    loadDogamOwned() {
    ...
    stageAtColumn(row, columnStart) {
      const band = row.bands.find(
        (b) => columnStart >= b.from && columnStart + 9 <= b.to,
      );
    ...
    fillCurrentTime() {
      let targetRecord = this.killsRecords.find((record) =>
        [record.month, record.day, record.hour, record.minute].every(
    ...
    resetExcludingLastKillsRecord() {
      if (this.killsRecords.length > 0) {
        const lastRecordCandidate =
          this.killsRecords[this.killsRecords.length - 1];
    ...
    saveKillsRecords() {
      localStorage.setItem(
        this.killsLocalStorageKey,
        JSON.stringify(this.killsRecords),
      );
    },
    loadKillsRecords() {
    ...
    weapon_speed_cal: function () {
      let equip_speed =
        (1.0 -
          ...
          this.personality -
          this.secret / 100 -
          this.union_speed / 100 -
          this.pet_equipment / 100 -
          (this.final_speed / this.weapon_speed) *
...
.rune-matrix th,
.rune-matrix td,
.rune-option-table th,
.rune-option-table td {
  padding: 6px 5px;
}

.rune-matrix thead th,
.rune-option-table thead th {
  font-weight: normal;
  line-height: 1.3;
}
...
.dogam-set-table th:nth-child(1),
.dogam-set-table td:nth-child(1) {
  width: 16%;
}
.dogam-set-table th:nth-child(3),
.dogam-set-table td:nth-child(3) {
  width: 22%;
}
.dogam-set-table th:nth-child(4),
.dogam-set-table td:nth-child(4) {
  width: 15%;
}
.dogam-set-table th:nth-child(5),
.dogam-set-table td:nth-child(5) {
  width: 13%;
}

.dogam-box-table th:nth-child(1),
.dogam-box-table td:nth-child(1) {
  width: 14%;
}
.dogam-box-table th:nth-child(3),
.dogam-box-table td:nth-child(3) {
  width: 24%;
}
.dogam-box-table th:nth-child(4),
.dogam-box-table td:nth-child(4) {
  width: 13%;
}
.dogam-box-table th:nth-child(5),
.dogam-box-table td:nth-child(5) {
  width: 13%;
}

.dogam-chonbi-table th:nth-child(1),
.dogam-chonbi-table td:nth-child(1) {
  width: 20%;
}
.dogam-chonbi-table th:nth-child(3),
.dogam-chonbi-table td:nth-child(3) {
  width: 15%;
}
.dogam-chonbi-table th:nth-child(4),
.dogam-chonbi-table td:nth-child(4) {
  width: 15%;
}
...
.formation-preset-save,
.formation-preset-load {
  flex: 0 0 auto;
  font-size: 12px;
  padding: 5px 10px;
}
...
.equip-grade-table th,
.equip-grade-table td {
  padding: 6px 8px;
}
...
.unique-timeline-table th,
.unique-timeline-table td {
  padding: 6px 8px;
}
